...
router.get('/layout', async (req, res) => {
  try {
    ...
    if (!userId || !practiceId) return res.status(400).json({ error: 'practiceId and auth required' });
...
router.post('/layout', async (req, res) => {
  try {
    ...
    if (!practiceId || !layout || !widgets) return res.status(400).json({ error: 'practiceId, layout and widgets are required' });
    ...
    const [inserted] = await db.insert(userDashboards).values({ userId, practiceId, layout, widgets, isActive: true as any, createdAt: new Date(), updatedAt: new Date() }).returning();
