...
// Manager BI Hooks
export const useManagerMetrics = (date: string = new Date().toISOString().split('T')[0]): UseQueryResult<any> => {
  const user = useAuthStore(state => state.user);
  const currentPractice = useAuthStore(state => state.currentPractice);

  return useQuery({
    queryKey: ['manager-metrics', currentPractice?.id, date],
    queryFn: () => analyticsAPI.getManagerMetrics(currentPractice!.id, date),
    enabled: !!user && !!currentPractice,
...
export const useOperationalInsights = (dateRange: string = '7d'): UseQueryResult<any> => {
  ...
  return useQuery({
    ...
    queryFn: () => analyticsAPI.getOperationalInsights(currentPractice!.id, dateRange),
    enabled: !!user && !!currentPractice,
    staleTime: 5 * 60 * 1000,
    refetchInterval: 10 * 60 * 1000,
  });
